...
int t_query(char q_id[IDLEN + 2])
{
	...
	if( num )
	{
		sprintf(genbuf, "Ä¿Ç°ÔÚÏß£º[[1;32mÑ¶Ï¢Æ÷£º([36m%s[32m) ºô½ÐÆ÷£º([36m%s[32m)[m] ",
		        canmsg(&uin)?"´ò¿ª":"¹Ø±Õ",
	...
	prints("ÎÄÕÂÊý: [[1;32m%d[m]([1;33m%s[m) ¾­ÑéÖµ£º[[1;32m%d[m]([1;33m%s[m)¡£½±ÕÂÊý: [%d] ÉúÃüÁ¦£º[[1;32m%d[m]\n",
	       lookupuser.numposts,cnumposts(lookupuser.numposts),
	...
	if(permptr[4]=='M'&&permptr[5]=='A'&&permptr[6]=='0'&&permptr[8]=='S')
		prints("  [[1;35mÕ¾Îñ×Ü¹Ü[0m]\n"£©;
		       else if(permptr[8]=='S')
		       prints("  [[1;35mÏµÍ³Î¬»¤[0m]\n"£©;
		              else if(permptr[5]=='A')
			              prints("  [[1;35mÕÊºÅ×Ü¹Ü[0m]\n"£©;
			                     else if(permptr[6]=='0')
				                     prints("  [[1;35mÌÖÂÛÇø×Ü¹Ü[0m]\n"£©;
				                            else if(permptr[4]=='M')
					                            prints("  [[1;35mÌÖÖÇÄÒÍÅ³ÉÔ±[0m]\n"£©;
#endif
