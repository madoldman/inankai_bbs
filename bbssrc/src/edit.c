...
void msgline()
{
	...
	sprintf(buf2,"[[32m%s[33m][[32m%4.4d[33m,[32m%3.3d[33m][[32m%d[33m] [[32m%2s[33m]", insert_character ? "²åÈë" : "¸ÄÐ´", currln + 1, currpnt + 1,linelen-1,enabledbchar?"Ë«":"µ¥");
	strcat(buf, buf2);
...
int write_posts()
{
	...
	if(if_exist_title(ptr))
		return; /* add by yiyo for Ê®´ó°´ÈËÊýÍ³¼Æ */
	if(!strcmp(currboard,"Blessing"))
...
void write_header(FILE *fp, int mode)
{
	...
	char    uid[20];
	char    uname[NAMELEN];
	char    ipshow[60];//add by yiyo·¢±íÎÄÕÂipÏÔÊ¾
	...
	for (i = 1; i <= MAXSIGLINES; i++)
	{
		if (fgets(inbuf, sizeof(inbuf), sigfile))
		{
			if( !HAS_PERM(PERM_SYSOP) && strstr( inbuf, genbuf ) )
				continue;
			/* add by yiyo·ÀÖ¹ÀûÓÃÇ©ÃûµµÒþ²ØIPÀ´Ô´ */
			if (inbuf[0] != '\n')
				valid_ln = i;
...
int badwords=0; //add for 'FILTER' Èç¹û³öÏÖÃô¸Ð×ÖÔòÎª1
...
int write_file(char *filename,int write_header_to_file,int addfrom, int sure)
{
	...
	if (uinfo.mode != CCUGOPHER)
	{
		...
		{
                        if(pattern==1){
                            strcpy(p_buf,"L.¿ÉÒÔÀ², A.Ìø¹ý¸ÃÎÊ, E.ÔÙ±à¼­? [L]: ");
			}
                        else{
			...
			move(5,0);
			prints(" ±¾Õ¾·¢ÎÄÐëÖª£º\n ÄªÒªÒ»ÎÄ¶à·¢£¨³¬¹ý5¸ö°åÃæ¿ÉÊÇ»á±»·âÈ«Õ¾µÄÅ¶£©\n ÄªÒª·¢±í¿ÕÎÄ¡¢ÐîÒâ¹àË®ÎÄÕÂ\n ÄªÒª·¢±í´øÓÐÈèÂîÎêÈèÐÔÖÊµÄÎÄÕÂ\n ÄªÒª·¢±íÕÒÑ°Ç¹ÊÖµÄÎÄÕÂºÍÎ¥·¨¹ú¼Ò·¨¹æµÄÎÄÕÂ£¨»á´¦ÒÔ¼«ÐÌ...É¾³ýÕÊºÅ!£©\n");
			}
		}
	...
	while (p != NULL)
	{
		...
		if (aborted != -1)
		{
			...
			{
				//********************* modified by silencer 02.5.31 ipÒþ²Ø  **************
#ifdef DEF_HIDE_IP_ENABLE
				...
				/*
					fprintf(fp,
					       "[1;36m¡ù ÐÞ¸Ä:¡¤%s ÓÚ %16.16s ÐÞ¸Ä±¾ÎÄ¡¤[FROM:     *     ][m\n",
	        ...
	        &&addfrom!=0&&aborted!=-1&&write_header_to_file)
	{
		...
		fprintf(fp, "[m[1;%2dm¡ù À´Ô´:¡¤%s %s¡¤[FROM: %-.20s][m ",
		        color, BoardName, BBSHOST, (noidboard) ? "ÄäÃûÌìÊ¹µÄ¼Ò" : ipshow);
	}
	if (aborted != -1)
		fclose(fp);
	currline = NULL;
...
#define NO_ANSI_MODIFY  if(no_touch) { warn++; break; }
			...
			if(currpnt > linelen - 4)
	{
			...
			//add end by yiyoÐÐÎ²ÊäÈëTAB»òCtrl-I£¬¾Í»á³öÏÖËÀÑ­»·µÄÎÊÌâ
			do
			{
				insert_char(' ');
			}
	...
	while (ch != EOF)
	{
		...
		case Ctrl('W'):
						/*
								...
								clear();
								move(5,0);
						                prints("          ¶Ô²»Æð£¬Ä¿Ç°±¾Õ¾´¦ÓÚÖ»¶ÁÎ¬»¤×´Ì¬¡£\n");
						                pressreturn();
						                return -1;
						        }//add by yiyo È«Õ¾Ö»¶ÁÉèÖÃ
						*/
...
/* add by bluetent ¹Ø¼ü×Ö¹ýÂË(FILTER) */
