	...
	while (c > 0)
	{
		...
		if (*bptr == '/')
		{
			...
			case 't':
				...
				sprintf(genbuf, "[1;44;33m ·¿¼ä£º [32m%-12.12s  [5;31;44m%6.6s[0;1;44;33m»°Ìâ£º[36m%-49.49s[m", chatname,
...
void query_user(char *arg)
{
	...
	{
		sprintf(msg, "%sÉÏ´ÎÉÏÕ¾Ê±¼ä:  [[1;32m%s[m] ",
	...
	printchatline(msg);
	sprintf(msg,
	        "ÐÅÏä: [[1;5;32m%2s[m]£¬¾­ÑéÖµ: [[1;32m%d[m]([1;36m%s[m)"
	        " ±íÏÖÖµ: [[1;32m%d[m]([1;36m%s[m) ÉúÃüÁ¦: [[1;32m%d[m]",
	...
	else
	{
		...
		sprintf(msg,"[1;37mÀëÕ¾Ê±¼ä: [[1;32m%s[1;37m][m",datestring);
	}
	printchatline(msg);
}
...
void set_rec()
{
	...
	/* modified by yiyo ½â¾öÓÃ»§½ø¶à¸öÁÄÌìÊÒÂ¼Òô²»È«µÄÎÊÌâ */
	...
	if (recflag == 0)
	{
		...
		clrtoeol();
		transPERstr(buftopic,tmptopic);
		sprintf(genbuf, "[1;44;33m ·¿¼ä£º [32m%-12.12s  [5;31;44m%6.6s[0;1;44;33m»°Ìâ£º[36m%-49.49s[m", chatname, (recflag == 1) ? "[Â¼Òô]" : "      ",tmptopic);
		prints("%s",genbuf);
