...
static char addressitems[NADDRESSITEM][16]={
            "QQ号码",
            "常用Email",
            "固定电话",
            "呼机/手机",
...
int addressbook()
{
	...
	while(1)
	{
		...
		case 'a':
			...
			move(5,0);
			clrtobot();
			showaddressbook(items, addressbookmode(currentuser.userid, buf));
			pressanykey();
			break;
		case 'B':
		case 'b':
			sethomefile(filename, currentuser.userid, "addressbook");
			readaddressbook(filename, items);
			...
			for(i=0;i<NADDRESSITEM;i++)
			{
				...
				getdata(NADDRESSITEM+5,0,str,items[i]+1,60,
				        DOECHO, items[i][0]?NA:YEA);
				if(items[i][0]<'0'||items[i][0]>'3')
					items[i][0]='3';
				sprintf(buf,"公开级别 (0)所有人 (1)与我为友者 (2)我的好友 (3)不公开 [%c]",items[i][0]);
				if(items[i][1])
					getdata(NADDRESSITEM+6,0,buf, str, 3, DOECHO, YEA);
				if(str[0]<'0'||str[0]>'3')
					str[0]=items[i][0];
				items[i][0]=str[0];
			}
			...
			showaddressbook(items, 1);
			prints("=================================\n");
			getdata(NADDRESSITEM+5,0,"以上资料是否正确, 按 Q 放弃 (Y/Quit)? [Y]:",
			        str, 3, DOECHO, YEA);
			if(str[0]!='Q'||str[0]=='q')
...
int isoverride(struct override *o, char *id)
{
...
int inoverride(char *who, char *owner, char *file)
{
	...
	if(search_record(buf, &o, sizeof(o), isoverride, who)!=0)
		return 1;
	return 0;
}

int addressbookmode(char *me, char *him)
{
...
int showaddressitem(char *item, int i, int mode)
{
	//	int f;
	if(item[0]==0)
	{
...
int showaddressbook(char items[][STRLEN], int mode)
{
	int i;
	for(i=0;i<NADDRESSITEM;i++)
...
int readaddressbook(char *filename, char items[][STRLEN])
{
	int i=0;
	FILE *fp;
	char *ptr;
...
int saveaddressbook(char *filename, char items[][STRLEN])
{
	int i;
	FILE *fp;
	fp=fopen(filename, "w");
