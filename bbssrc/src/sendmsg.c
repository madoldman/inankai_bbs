...
int msg_blocked=0;//add by yiyo ·¢ÎÄÕÂÊ±ÔİÊ±ÆÁ±ÎÏûÏ¢
...
int get_msg(char* uid, char* msg, int line)
{
	...
	//	prints("ËÍÒôĞÅ¸ø:%-13s  ÇëÊäÈëÒôĞÅÄÚÈİ,Ctrl+Q»»ĞĞ",uid); //add for msgs
...
int do_sendmsg(struct user_info* uentp, char msgstr[256], int mode, int userpid)
{
	...
	*(timestr + 8) = '\0';
	...
	{
		sprintf(msgbuf, "[0;1;5;44;33mÕ¾³¤ ÓÚ[36m %8.8s [33m¹ã²¥£º[m[1;37;44m%-57.57s[m[%05dm\n",
...
*/
	...
	/* add by yiyo Ë«·½×Ô¶¯Ó¦´ğÔì³ÉÑ¶Ï¢·´µ¯ÎÊÌâµÄÒ»ÖÖ½â¾ö·½·¨ */
	if(flood_msg(msgbuf, userpid))
	{
		move(1, 0);
		clrtoeol();
		prints("´íÎó! Äú¸Õ²ÅÒÑ¾­·¢¹ıÕâ¸öÏûÏ¢ÁË...\n");
		refresh();
		sleep(1);
		return -1;
	}
			...
			if (good_id == YEA && canmsg(uin))
			{
				...
				if (!strstr(msgbuf, "°ïÄã"))
					sleep(1);
				    		 ...
				    		 sprintf(msgbuf,"[1;32mÇëÏÈ½â³ıÓ©Ä»Ëø¶¨ÔÙ»Ø¸´Ñ¶Ï¢.:P[m");
				    	 }
				
				else*/
				{
					sprintf(msgbuf, "[1;32mÕÒ²»µ½·¢Ñ¶Ï¢µÄ %s! Çë°´ÉÏ:[^Z ¡ü] "
					        "»òÏÂ:[^A ¡ı] »òÆäËû¼üÀë¿ª.[m", usid);
					move(line + 1, 0);
					clrtoeol();
					refresh();
					prints("%s", msgbuf);
...

			{
				...
				else if(ch==Ctrl('R')||ch=='R'||ch=='r'||ch==Ctrl('Z'))
				{
					...
					if (ptr == NULL)
						...
						good_id = NA;
					/*modified by tdhlshx Ëø¶¨×´Ì¬²»ÄÜ»Ø¸´Ñ¶Ï¢03.11.8*/
					else
					{
						strcpy(usid, ptr);
						uin = t_search(usid, send_pid);
...
/* add by bluetent ÌáÈ¡Ñ¶Ï¢ 2003.01.18 */
int AbstractMsg()
{
	FILE *fp, *fp2;
	...
	{
		move (10,10);
		prints("ÄúÃ»ÓĞÈÎºÎÑ¶Ï¢¡­¡­");
		pressanykey();
		return FULLUPDATE;
	}
	sethomefile(buf, currentuser.userid, "abstractmsg");
	ListFileEdit(buf,"ÄúÏëÌáÈ¡ÄÄĞ©ÓÃ»§µÄÑ¶Ï¢£¿Çë±à¼­´ËÁĞ±í(Ä¿Ç°Ö§³Ö100¸öÓÃ»§)");
	if(fp2=fopen(buf,"r"))
	{
		i=0;
	...
	while(abstract[i][0]!='\0')
	{
		...
		//		system(buf);
		sprintf(buf3, "ÒÑÌáÈ¡µÄÑ¶Ï¢{%s}", abstract[i]);
	...
	clear();
	move(10,10);
	prints("ÒÑ¾­½«ÌáÈ¡³öµÄÑ¶Ï¢·¢ËÍÖÁÄúµÄĞÅÏä");
	pressanykey();
	return FULLUPDATE;
}

/* add by bluetent Ñ¶Ï¢ËÑË÷ */
int SearchMsg()
{
	FILE *fp, *fp2;
	...
	getdata(3,0,"ÇëÊäÈëÒªËÑË÷µÄ¹Ø¼ü×Ö£º",key,40,DOECHO,YEA);
	if( key[0] == '\0' )
		return 0;
	...
	fclose(fp);
	fclose(fp2);
	sprintf(genbuf, "ÒÑÌáÈ¡µÄÑ¶Ï¢{¹Ø¼ü×Ö:%s}", key);
