		...
		for (i = 1; i <= MAXQUERYLINES; i++)
		{
			if (fgets(pbuf, sizeof(pbuf), pf))
			{
				for (j = 0; j < strlen(pbuf); j++)
					if (pbuf[j] == '\015')
						continue;//add by yiyo WWW·½Ê½ÏÂÐÞ¸ÄËµÃ÷µµÔÚtelnetÏÂÕÛÐÐ
					else if (pbuf[j] != '\033')
						outc(pbuf[j]);
					else
					{
...
begin_show:
	...
	prints("ÉÏ ´Î ÔÚ: [[1;32m%s[m] ´Ó [[1;32m%s[m] µ½±¾Õ¾Ò»ÓÎ¡£\n",
	       ...
	       hideip?"ÄÏ¿ªËÞÉáÄÚÍø":lookupuser.lasthost);
#else
	        ...
	        &&(lookupuser.userlevel&PERM_RECLUSION))
	{
		...
		sprintf(life,"[\033[36m¹éÒþÖÐ, »¹ÓÐ%dÌì\033[m]", (Reclusion-(time(0)-lookupuser.lastlogout)/86400)>0?(Reclusion-(time(0)-lookupuser.lastlogout)/86400):0);
	}
	else
		sprintf(life,"");
#ifdef ALLOWGAME
	prints("ÒøÐÐ´æ¿î: [[1;32m%dÔª[m] Ä¿Ç°´û¿î: [[1;32m%dÔª[m]([1;33m%s[m) ¾­ÑéÖµ: [[1;32m%d[m]([1;33m%s[m)\n",
	       lookupuser.money,lookupuser.bet,
	       cmoney(lookupuser.money-lookupuser.bet),exp,cexp(exp));
	prints("ÎÄ ÕÂ Êý: [[1;32m%d[m]([1;33m%s[m) ½±ÕÂÊý: [[1;32m%d[m]([1;33m%s[m) ÉúÃüÁ¦: [[1;32m%d[m]\n",
...
#else
	prints("ÎÄ ÕÂ Êý: [[1;32m%d[m]([1;33m%s[m) ¾­ÑéÖµ: [[1;32m%d[m]([1;33m%s[m) ÉúÃüÁ¦: [[1;32m%d[m]\n",
		   lookupuser.numposts,cnumposts(lookupuser.numposts),
...
#ifdef ALLOW_MEDALS
	{
		...
		int tmp1 = 0;  // Efan: ¸ù¾ÝÎ»ÖÃÅÐ¶Ï½±ÕÂÀàÐÍ
		for (i = 1; i <= 7; ++i) {
	...
	prints("ÓÃ»§È¨ÏÞ£º[%s][[1;35mÎÒ°®ÄÏ¿ªÕ¾Îñ×é[m]",permptr);
	else prints("ÓÃ»§È¨ÏÞ: [%s][[1;35mÕ¾Îñ×Ü¹Ü[m]",permptr);
	else if(permptr[8]=='S')
	prints("ÓÃ»§È¨ÏÞ: [%s][[1;35mÏµÍ³Î¬»¤[m]",permptr);
	else if(permptr[5]=='A')
	prints("ÓÃ»§È¨ÏÞ: [%s][[1;35mÕÊºÅ×Ü¹Ü[m]",permptr);
	else if(permptr[6]=='O')
	prints("ÓÃ»§È¨ÏÞ: [%s][[1;35mÌÖÂÛÇø×Ü¹Ü[m]",permptr);
	else if(permptr[4]=='M')
	prints("ÓÃ»§È¨ÏÞ: [%s][[1;35mÃÀÊõ×Ü¹Ü[m]",permptr);
	else
	*/
	prints("ÓÃ»§È¨ÏÞ: [%s]",permptr);
	...
	if ((lookupuser.userlevel&PERM_BOARDS)
		...
		prints(" ÏÖÈÎ\033[36m%s\033[m°å°åÎñ\n",bmlist);
	else
		prints("%s\n", life);
#endif
	t_search_ulist(&uin, t_cmpuids, tuid, YEA, NA);
	//#if defined(QUERY_REALNAMES)
	//   if (HAS_PERM(PERM_SYSOP)) prints("ÕæÊµÐÕÃû: %s \n", lookupuser.realname);
	// modified by yiyo ²éÑ¯ÍøÓÑ²»ÔÙÏÔÊ¾ÕæÊµÐÕÃû
	//#endif
	        ...
	        && uinfo.mode != FRIEND && uinfo.mode != GMENU)
	{
		...
		prints("\033[0;1;44;37m%s%s%s\033[m", (num) ?"ÁÄÌì[\033[32mt\033[37m] ËÍÑ¶Ï¢[\033[32ms\033[37m] ": "",dashd(fnPersonal)? "½øÈë¸öÈËÎÄ¼¯[\033[32mx\033[37m] ": "","¼ÄÐÅ[\033[32mm\033[37m] ÉèÎªºÃÓÑ/»µÈË[\033[32mo\033[37m/\033[32mr\033[37m]  (ÆäËûÈÎÒâ¼üÀë¿ª)\033[m");
		...
		case 'R':
			...
			sprintf(buf, "È·¶¨Òª°Ñ %s ¼ÓÈë%sÃûµ¥Âð", lookupuser.userid,desc);
			clear();
			move(t_lines - 1, 0);
	...
	}//add by yiyo ²éÑ¯ÍøÓÑºó¿ÉÁ¢¼´·¢ËÍÏûÏ¢»òÕßÐÅ¼þ
	//      pressanykey();
	uinfo.destuid = 0;
	return 0;
}
...
int showmedal (unsigned int num)
{
	...
	for (i = 0; i < 7; ++i) {
		if (nummedal [i] != 0)
		{
			prints ("%50s\033[1;37m%10d Ã¶\033[m\n", cmedal (i + 1), nummedal [i]);
		}
	}
	pressanykey ();
	return 0;
...
char * cmedal2 (unsigned int original, int num, int place)
{
	...
	strcpy (retstr, "");
	if (num <= 1)	// Efan: ½±ÕÂÖÖÀàÊýÐ¡ÓÚ 2 µÄÇé¿ö
		return cmedal (place);
	for (i = 7; i > 0; --i) {
		...
		if (tmp != 0) {
			 ...
			 */
			if (i != 2)
				sprintf (tmpstr, "\033[1;3%dm¡ï", 8 - i);
			else
				sprintf (tmpstr, "\033[0;31m¡ï");
			strcat (retstr, tmpstr);
		}
	}
	strcat (retstr, "\033[1;33m °´ \033[1;37mi \033[1;33m¼ü²é¿´½±ÕÂ\033[m");
	return retstr;
}
...
int count_wwwguest(struct user_info* uentp)
{
	...
	if ( uentp->mode > 20000 )//thunder wwwÔÚÏß
		count++;
	return 1;
}
/* add end */
...
int num_alcounter()
{
	...
	alcounter(NULL);
	return;
} //modified by yiyo ¸Ä½ønum_alcounterº¯Êý
int num_useshell()
{
	count_useshell(NULL);
...
int talkreply()
{
	...
	sprintf(inbuf, "ÄãÏë¸ú %s %sÂð£¿ÇëÑ¡Ôñ(Y/N/A/B/C/D)[Y] ",page_requestor,(five)?"ÏÂÎå×ÓÆå":"ÁÄÁÄÌì");
#endif
...
int do_list(char* modestr)
{
	...
	{	/* Èç¹ûÊÕµ½ Msg µÚÒ»ÐÐ²»ÏÔÊ¾¡£ */
		...
		{
			sprintf(buf, "[%d·âÐÂÐÅ]", mail_unread());
...
int addtooverride(char* uident)
{
	...
	if (get_num_records(buf, sizeof(struct override)) >= n)
	{
		move(t_lines - 2, 0);
		clrtoeol();
		prints("±§Ç¸£¬±¾Õ¾Ä¿Ç°½ö¿ÉÒÔÉè¶¨ %d ¸ö%s, Çë°´ÈÎÒâ¼ü¼ÌÐø...", n, desc);
		igetkey();
		move(t_lines - 2, 0);
		clrtoeol();
		return -1;
...
override_title()
{
	...
	if (chkmail()&&mail_unread()>0)
	{
		sprintf(genbuf, "[%d·âÐÂÐÅ]", mail_unread());
	}
	else
...
struct user_info* t_search(char* sid, int pid)
{
	...
	*/
	...
	if (mCost < 100000)
		...
		return isreject(cur) ? NULL : tmp;  //modified by yiyo  ½â¾öÃ÷Ã÷¿´µ½¶Ô·½ÔÚÏßÈ·²»ÄÜ¸øËû·¢ÏûÏ¢µÄÎÊÌâ
}
