...
*/
/* $Id: bbspop3d.c,v 1.1.1.1 2008-11-23 08:13:23 madoldman Exp $ */
     ...
     for (i = 0; i < totalnum; i++) {
       if (index(fcache[i].owner, '@') == NULL) {
         if (ptr = strchr(fcache[i].owner, ' '))
           *ptr = '\0';
         strcat(fcache[i].owner, M_ADDR);
       }
   /* add by brew */
   /* Sender check when nickname has '@' */
	else
	{
	   if((ptr=strstr(fcache[i].owner, " ("))!=NULL)
	   {//maybe the wrong message...
		saveCh=*ptr;
	        *ptr = '\0';
		sprintf(genbuf, "home/%c/%s", toupper(fcache[i].owner[0]), fcache[i].owner);
		if (stat(genbuf, &st) != -1) 
		{
	  		strcat(fcache[i].owner, M_ADDR);	
		}
...
main(int argc,char **argv)
{
  ...
  //            ioctl(n, TIOCNOTTY, 0) ; 
	...
	retry = 0;
  	  ...
  	  if(checkaddr(fsin.sin_addr, sock) < 0)
	  {
		  close(sock);
		  continue;
	  }
...
int Retr()
{
     ...
     outs(genbuf);
     sprintf(genbuf, "To: %s%s", currentuser.userid,M_ADDR);
     outs(genbuf);
				...
				if(j==0&&fd>=0)
					if(fork()==0)
					{
						sprintf(str, "add\t%s\t%d\t%s", inet_ntoa(addr), addrcheck[i].n,
						        ctime(&timenow));
						write(fd, str, strlen(str));
#define WARNING "对不起, 连接将封闭3分钟。请不要不断连接冲击本站\n"
