/*$Id: bbsfadm.c,v 1.2 2008-12-19 09:55:09 madoldman Exp $
*/
...
int main()
{
        ...
        if(start<0)
                start=0;
        printf("<center> %s -- 附件管理 [附件数：%d，已占用硬盘空间：%d字节，平均每个附件占用：%d字节]<hr color=green>\n", BBSNAME, total, totalsize, totalsize/total);
        printf("<table width=610 border=1><tr><td>序号<td>文件名<td>上载者<td>板面<td>时间<td>大小<td>说明<td>备注<td>管理\n");
        ...
        if(start>0)
                printf("[<a href=bbsfadm?start=%d>上一页</a>] ", start-19);
        if(start<total-20)
                printf("[<a href=bbsfadm?start=%d>下一页</a>] ", start+19);
