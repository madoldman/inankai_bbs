/*$Id: bbszone.c,v 1.2 2008-12-19 09:55:09 madoldman Exp $
*/
...
int main()
{
	...
	strsncpy(zonename, getparm("zonename"), 32);
        ...
        for(i=0; i<MAXBOARD; i++) 
	{
                ...
                if(x->filename[0]<=32 || x->filename[0]>'z')
                        continue;
                if(!has_read_perm(&currentuser, x->filename) && !HAS_PERM(PERM_SYSOP))
                        continue;
	///	printf("name:%s\n",x->title);
        //        if(!strchr(seccode[sec1], x->title[0]))
        //                continue;
		if(!(x->flag & CHILDBOARD_FLAG) && strcmp(x->owner,zonename)!=0)
			continue;
		if(strcmp(x->owner,zonename)!=0)
			continue;
